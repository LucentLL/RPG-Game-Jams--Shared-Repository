...
/**
 * No layout box may run longer than this many tiles. Whole tiles, so the
 * repeating apron and stands textures never seam.
 *
 * THIS NUMBER NO LONGER GUARDS WHAT IT WAS WRITTEN TO GUARD, and it is worth
 * knowing why before anyone tunes it again. It was chosen when every quad
 * carried a CSS `filter`, and a filtered element IS one render surface — one
 * texture, subject to GL_MAX_TEXTURE_SIZE, dropped whole if it will not fit.
 * Segmenting was the right answer to that.
 *
 * The filters are gone now (body.fp-lite). An UNFILTERED transformed element is
 * a TILED picture layer: the compositor allocates only the tiles that intersect
 * the view, so its layout size is irrelevant and the texture cap never applies.
 * Every size-based mitigation after the filters left — including a version of
 * this file that cut the ground to one tile a panel — was aimed at a limit that
 * had stopped binding, and it made things worse: `preserve-3d` (battle.css)
 * forces ONE COMPOSITED LAYER PER QUAD, so finer segmentation buys nothing and
 * costs layers, on the device with the least tile memory to spend.
 *
 * What actually fails is the global tile budget, which is why the SKY — a
 * viewport-sized layer with no transform, no 3D context and no filter — drops
 * too, and why it only drops while MOVING: standing still holds every layer's
 * on-screen scale constant, so the scene rasters once and is then merely
 * re-composited. Walking sweeps every quad's effective scale (up to ~8x across
 * six tiles at this lens) and forces the compositor to re-raster all of them.
 *
 * The fix is fewer layers, not smaller ones — or no layers at all, which is
 * what gl-world.js is for. @see HANDOFF-RENDERER.md and perf-arena-mobile.
 */
export const SEG_T = 3;
/** The ground panels' own segment size. Kept as its own name because the ground
 *  is the one surface worth tuning separately if it ever matters again — but it
 *  tracks SEG_T now, for the reason above. */
export const SEG_GROUND = SEG_T;
